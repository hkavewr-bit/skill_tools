...
| 前端交互与规范（有前端） | [前端设计](references/frontend.md) | 交互、状态、权限、数据需求确认；从已确认页面提炼前端设计规范，PRD 已同步 |
...
| 小步开发、测试、验收 | [实施与验证](references/implementation.md) | WIP=1；当前业务切片测试和文档同步通过后自动提交到当前版本 PR，用户验收后才进入下一切片；业务阶段结束后完成质量检查点 |
...
文档模板、命名与使用时机见 [状态与文档维护](references/state-and-docs.md)。用户调用本 Skill 开始项目开发，即授权在项目已有 Git 仓库和远程仓库中，为当前交付版本维护一个开发分支和一个 PR：每轮业务切片通过后自动提交、推送并创建或更新该 PR。该授权不包含合并 PR、发布、部署、覆盖远程历史或安装社区依赖；这些动作仍需单独授权。仓库、远程或认证不可用时记录阻碍，不伪造 PR 已提交。
